...
// build/dev/javascript/portfolio/posts.mjs
...
var FILEPATH = "src/portfolio.gleam";
...
function view_footer() {
  return footer(
    ...
    toList([
      div(
        toList([class$("social-links")]),
        toList([
          a(
            toList([
              href("https://github.com/eltonsst"),
              target("_blank"),
          ...
          a(
            toList([
              href("https://linkedin.com/in/elton-stafa-bb802414a"),
              target("_blank"),
              rel("noopener noreferrer")
            ]),
            toList([text3("LinkedIn")])
          ),
          a(
            toList([
              href("https://x.com/eltonsst"),
              target("_blank"),
              rel("noopener noreferrer")
            ]),
            toList([text3("X")])
          ...
          a(
            toList([
              href("https://instagram.com/eltonsst"),
              target("_blank"),
              rel("noopener noreferrer")
            ]),
            toList([text3("Instagram")])
          )
        ])
      )
    ])
...
function title(title2) {
  return h2(toList([]), toList([text3(title2)]));
}
...
function view_header_link(target2, current, text4) {
  let _block;
  if (target2 instanceof Posts && current instanceof PostById) {
    _block = true;
  } else {
    _block = isEqual(current, target2);
  }
  let is_active = _block;
  return li(
    toList([classes(toList([["active", is_active]]))]),
    toList([a(toList([href2(target2)]), toList([text3(text4)]))])
  );
}
      ...
      return article(
        ...
        toList([
          h3(
            ...
            toList([
              a(
                toList([href2(new PostById(post.id))]),
                toList([text3(post.title)])
              )
            ])
          ),
          p(toList([]), toList([text3(post.summary)]))
        ])
      );
    }
  );
  let posts2 = _block;
  return prepend(hr(toList([])), prepend(title("Posts"), posts2));
}
function link(target2, title2) {
  return a(toList([href2(target2)]), toList([text3(title2)]));
}
...
function init2(_) {
  ...
  let posts2 = _block$1;
  let model = new Model(posts2, route);
  let effect = init(
    (uri) => {
      return new UserNavigatedTo(parse_route(uri));
    }
  );
  return [model, effect];
}
function last_post(posts2) {
  let $ = (() => {
    let _pipe = posts2;
    let _pipe$1 = keys(_pipe);
    return max(_pipe$1, compare);
  })();
  if ($ instanceof Ok) {
    let max_key = $[0];
    return map_get(posts2, max_key);
  } else {
    return new Error(void 0);
  }
}
function view_index(model) {
  ...
  return toList([
    ...
    (() => {
      if (result_post instanceof Ok) {
        let post = result_post[0];
        return article(
          toList([class$("last-post")]),
          toList([
            h3(
              toList([]),
              toList([
...
function view2(model) {
  return div(
    ...
    toList([
      ...
      nav(
        toList([class$("secondary-nav")]),
        toList([
          ul(
            toList([]),
            toList([view_header_link(new Posts(), model.route, "Posts")])
          )
        ])
      ),
      main(
...
function main2() {
  ...
  if (!($ instanceof Ok)) {
    throw makeError(
      ...
      { value: $, start: 361, end: 410, pattern_start: 372, pattern_end: 377 }
    );
  }
  return void 0;
}
